...
CONTAINS
  ...
  SUBROUTINE find_nearest_target_grid_element(point_lon_geo, &
    ...
    REAL (wp), INTENT(in) :: point_lon_geo    !< longitude in geographical system of input point
    REAL (wp), INTENT(in) :: point_lat_geo    !< latitude in geographical system of input point

    TYPE(target_grid_def), INTENT(IN) :: tg   !< structure with target grid description

    INTEGER (i8), INTENT(INOUT) :: start_cell_id !< ID of starting cell
    INTEGER (i8), INTENT(OUT) :: tg_el_ie        !< Index of target grid element nearest to the input point
    INTEGER (i8), INTENT(OUT) :: tg_el_je        !< Index of target grid element nearest to the input point
    INTEGER (i8), INTENT(OUT) :: tg_el_ke        !< Index of target grid element nearest to the input point
    ...
    CASE(igrid_icon)
      ...
      ! note that the ICON coordinates do not have the unit degree but radians
      target_geo_co%lon = point_lon_geo * deg2rad
      target_geo_co%lat = point_lat_geo * deg2rad

      ! transform the geographical coordinates of the point to cartesian coordinates
