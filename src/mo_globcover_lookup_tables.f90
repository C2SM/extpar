...
PUBLIC :: get_name_globcover_lookup_tables
PUBLIC :: globcover_look_up
PUBLIC :: corinecover_look_up
PUBLIC :: get_globcover_idx
PUBLIC :: get_corinecover_idx
PUBLIC :: globcover_legend
PUBLIC :: globcover_value

PUBLIC :: nclass_globcover
PUBLIC :: ilookup_table_globcover
PUBLIC :: i_extpar_lookup_table, i_extpar_test_lookup_table
PUBLIC :: name_lookup_table_globcover
PUBLIC :: z0_lt_globcover, lnz0_lt_globcover, plc_mn_lt_globcover, plc_mx_lt_globcover
PUBLIC :: lai_mn_lt_globcover, lai_mx_lt_globcover, rd_lt_globcover, skinc_lt_globcover
PUBLIC :: emiss_lt_globcover, rs_min_lt_globcover
...
REAL (KIND=wp) :: lnz0_lt_globcover(nclass_globcover)    !< corresponding natural logarithm of z0c_extpar_o
REAL (KIND=wp) :: plc_mn_lt_globcover(nclass_globcover)  !< lookup table landuse class to minimal plant cover
REAL (KIND=wp) :: plc_mx_lt_globcover(nclass_globcover)  !< lookup table landuse class to maximal plant cover
REAL (KIND=wp) :: lai_mn_lt_globcover(nclass_globcover)  !< lookup table landuse class to minimal leaf area index
REAL (KIND=wp) :: lai_mx_lt_globcover(nclass_globcover)  !< lookup table landuse class to maximal leaf area index
REAL (KIND=wp) :: rd_lt_globcover(nclass_globcover)      !< lookup table landuse class to root depth [m]
REAL (KIND=wp) :: skinc_lt_globcover(nclass_globcover)   !< lookup table landuse class to skin conductivity [W m-2 K-1]
REAL (KIND=wp) :: emiss_lt_globcover(nclass_globcover)   !< lookup table landuse class to surface thermal emissivity
REAL (KIND=wp) :: rs_min_lt_globcover(nclass_globcover)  !< lookup table landuse class to minimal stomata resistance
...
!REAL (KIND=wp) :: lnz0c_extpar_o(nclass_globcover)    !< corresponding natural logarithm of z0c_extpar_o
...
CONTAINS
  ...
  REAL (KIND=wp), INTENT(IN) :: skinc_lt_globcover(nclass_globcover)   !< lookup table landuse class to skin conductivity [W m-2 K-1]
    ...
    CALL get_corinecover_idx(lu,nclass)
       ...
       ! Test for true land points
          IF (nclass>=1 .AND. nclass<=22 .AND.nclass/=21) THEN
            ...
            IF (lu==111 .or. lu==112 .or. lu==121 .or. lu==122 .or. lu==123 .or. lu==124 .or. lu==133 ) purb   = 1.0  ! artificial surfaces
