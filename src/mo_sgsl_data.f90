...
! As it is unknown so far whether GLOBE or ASTER is chosen all parameters must be allocated in a second step.
    ...
    INTEGER(KIND=i4), INTENT (IN) :: ntiles       ! number of tiles: 36 for ASTER and 16 for GLOBE
    INTEGER(KIND=i4)              :: errorcode
    
    CALL logging%info('Enter routine: allocate_sgsl_data')
   ...
   ALLOCATE (sgsl_tiles_grid(1:ntiles), STAT = errorcode)
         IF (errorcode.NE.0) CALL logging%error('Cant allocate the vector sgsl_tiles_grid',__FILE__,__LINE__)
  ...
  SUBROUTINE fill_sgsl_data(raw_data_sgsl_path,sgsl_files,  &
    ...
    SELECT CASE (idem_type)                ! Also  could additionally be used for SELECT CASE (must first be read in)
      ...
      CASE (dem_gl)                                           ! GLOBE DEM is composed of 16 tiles
        CALL logging%info('GLOBE was used as DEM')
    ...
    END SELECT
     ...
     IF (errorcode.NE.0) CALL logging%error('Cant allocate the vector raw_sgsl_line',__FILE__,__LINE__)
     ALLOCATE (sgsl_tile_row(1:nc_tile), STAT = errorcode)
     IF (errorcode.NE.0) CALL logging%error('Cant allocate the vector sgsl_tile_row',__FILE__,__LINE__)
   ...
   SUBROUTINE allocate_raw_sgsl_fields(nrows,ncolumns)
     ...
     CALL logging%info('Enter routine: allocate_raw_sgsl_fields')
