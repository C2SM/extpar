...
PUBLIC :: find_rotated_lonlat_grid_element_index
  ...
  !!   and regular_grid_info%start_lat_reg
    ...
    REAL (KIND=wp)    :: point_lon_geo_var   !< longitude coordinate in geographical system of input point,
                                             !< variable for eventual shift


       undefined_integer = 0 ! set undefined to zero

    ! the regular grid domain could cross the 180/-180 meridian or the 0/360 meridian (depending on the chosen 
    ! value domain for the geographical coordinates)
    ! therefore the longitude coordinate of the point is shiftet to a value which is in any case greater than the 
    ! startlon_reg_lonlat value, 
