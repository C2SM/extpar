...
REAL(KIND=wp)     ::    &
  deghor               !< number of degrees per sector [deg]
...
!> local variables
  ...
  REAL(KIND=wp)     ::    &
    rlon_np, rlat_np,                & !< location of true North Pole in rot. coord.
...
!> parameters
  REAL(KIND=ireals), PARAMETER :: &  
    semimaj        = 6378137.0                        !< semimajor radius WGS 84
...
!> arguments
  INTEGER(KIND=i8),                   INTENT(IN) :: &
    nx, ny                    ! gridpoints number in the x and y dir
  INTEGER(KIND=i4),                   INTENT(IN) :: &
    nhori                     !  number of sectors
  REAL   (KIND=wp)   , DIMENSION(nx,ny), INTENT(IN) :: &
    h_hres                    ! topography [m]
  ...
  REAL   (KIND=wp)   ,                   INTENT(OUT):: &
    hor(nhori)                ! horizon angle [deg]

!> local variables
  INTEGER(KIND=i8) ::  &
    x0, y0,                   & ! center of the sector in x and y dir resp.
    i,  j , k,                & ! loop indices
    ngp,                      & ! number of gridpoints at each side of the sector center
    nzstat                      ! allocation status
  
  REAL(KIND=wp)       ::  &
    tmp,                      & ! temporary horizon [deg]
  ...
  REAL(KIND=wp)      , ALLOCATABLE :: &
    pcr(:,:),                 & ! polar coordinates (index1: angle, index2: radius)
    rcr(:,:),                 & ! rectangular coord.(index1: x    , index2: y     )
    dh(:),dn(:),              & ! distance between the sector center and the other points of interest in the sector
                                ! in [m] in the vertical and in the horizontal resp.
    rates(:)                    ! slopes between the sector center and the other points of interest in the sector

  INTEGER(KIND=i8), PARAMETER ::  &
...
!> local variables
  ...
  REAL(KIND=wp), DIMENSION(nhori+1)::   &
    ang_start,               & ! angle of the sector start  [rad]
  ...
  REAL(KIND=wp), DIMENSION(2*nhori+1):: &
    drhorizon                  ! duplicated horizon [rad]


  REAL(KIND=wp), PARAMETER::           &
    zepsilon = 2.0_wp * pi * 0.01_wp ! security parameter 
