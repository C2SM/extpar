MODULE mo_hiressoil_output_nc
  ...
  PUBLIC :: write_netcdf_hiressoil_icon_grid, &
  & write_netcdf_hiressoil_cosmo_grid


  TYPE(var_meta_info) :: hiressoil_mean_meta, &
       hiressoil_min_meta, &
       hiressoil_max_meta, &
       hiressoil_var_meta
  ...
  !----------------------------------------------------------------------------
  !> set global attributes for netcdf with hiressoiltype data
  SUBROUTINE set_global_att_hiressoiltype(global_attributes,icon_grid)
    ...
    global_attributes(4)%attributetext=TRIM(ydate)//'T'//TRIM(ytime)//' hiressoil_to_buffer'
    ...
    ENDIF
  END SUBROUTINE set_global_att_hiressoiltype
  !----------------------------------------------------------------------------

 !> create a netcdf file for data to the ICON grid
 SUBROUTINE write_netcdf_hiressoil_icon_grid(netcdf_filename, &
   &                                     icon_grid,      &
   &                                     tg,             &
   &                                     undefined       )
    ...
    WRITE(message_text,*) 'ENTER write_netcdf_hiressoil_icon_grid'
   ...
   !-------------------------------------------------------------
   ! define global attributes
   CALL set_global_att_hiressoiltype(global_attributes,icon_grid)
  ...
  CALL set_nc_grid_def_icon(grid_mapping)
    ...
    !-----------------------------------------------------------------
    ! FILL METADATA
    CALL create_hiressoil_meta(hiressoil_mean_meta,     dim_1d_icon,'hrs_mean', &
         &  'Mean')
    CALL create_hiressoil_meta(hiressoil_min_meta,     dim_1d_icon,'hrs_min', &
         &  'Minimum')
    CALL create_hiressoil_meta(hiressoil_max_meta,     dim_1d_icon,'hrs_max', &
         &  'Mean')
    CALL create_hiressoil_meta(hiressoil_var_meta,     dim_1d_icon,'hrs_var', &
         &  'Variance')

    !-----------------------------------------------------------------

    CALL netcdf_put_var(ncid,hiressoil_mean(1:icon_grid%ncell,1,1), hiressoil_mean_meta,     undefined)
    CALL netcdf_put_var(ncid,hiressoil_min(1:icon_grid%ncell,1,1),  hiressoil_min_meta,     undefined)
    CALL netcdf_put_var(ncid,hiressoil_max(1:icon_grid%ncell,1,1),  hiressoil_max_meta,     undefined)
    CALL netcdf_put_var(ncid,hiressoil_var(1:icon_grid%ncell,1,1),  hiressoil_var_meta,     undefined)
  ...
  CALL deallocate_icon_coor()
  
END SUBROUTINE write_netcdf_hiressoil_icon_grid

  !> create a netcdf file for data to the COSMO grid
 SUBROUTINE write_netcdf_hiressoil_cosmo_grid(netcdf_filename,&
   &                                     cosmo_grid,     &
   &                                     tg,             &
   &                                     undefined       )
  ...
  !-------------------------------------------------------------
  ! define global attributes
  CALL set_global_att_hiressoiltype(global_attributes)
  ...
  ! lon_geo_meta and lat_geo_meta
    ...
    !-----------------------------------------------------------------
    ! FILL METADATA
    CALL create_hiressoil_meta(hiressoil_mean_meta,     dim_2d_cosmo,'hrs_mean', &
         &  'Mean')
    CALL create_hiressoil_meta(hiressoil_min_meta,     dim_2d_cosmo,'hrs_min', &
         &  'Min')
    CALL create_hiressoil_meta(hiressoil_max_meta,     dim_2d_cosmo,'hrs_max', &
         &  'Max')
    CALL create_hiressoil_meta(hiressoil_var_meta,     dim_2d_cosmo,'hrs_var', &
         &  'Var')
   !-------------------------------------------------------------
   ! organize output
   ...
   !-----------------------------------------------------------------
    ...
    CALL netcdf_put_var(ncid,hiressoil_mean(1:cosmo_grid%nlon_rot,1:cosmo_grid%nlat_rot,1), hiressoil_mean_meta,     undefined)
    CALL netcdf_put_var(ncid,hiressoil_min(1:cosmo_grid%nlon_rot,1:cosmo_grid%nlat_rot,1), hiressoil_min_meta,     undefined)
    CALL netcdf_put_var(ncid,hiressoil_max(1:cosmo_grid%nlon_rot,1:cosmo_grid%nlat_rot,1), hiressoil_max_meta,     undefined)
    CALL netcdf_put_var(ncid,hiressoil_var(1:cosmo_grid%nlon_rot,1:cosmo_grid%nlat_rot,1), hiressoil_var_meta,     undefined)
 ...
 END SUBROUTINE write_netcdf_hiressoil_cosmo_grid
!-------------------------------------------------------------
SUBROUTINE create_hiressoil_meta(p_meta,diminfo,vname,lname)
...
END SUBROUTINE create_hiressoil_meta
!-------------------------------------------------------------
!-------------------------------------------------------------
...
!-------------------------------------------------------------
!-------------------------------------------------------------
END Module mo_hiressoil_output_nc
