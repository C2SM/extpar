MODULE mo_agg_hiressoil
  USE mo_logging
  USE mo_kind, ONLY: wp,i4

 
  USE mo_hiressoil_data,    ONLY: type_clay_heavy, &
  & type_silty_clay, &
  & type_clay_light, &
  ...
  USE mo_hiressoil_tg_fields, ONLY: fr_heavy_clay, &
  & fr_silty_clay, &
  & fr_light_clay, &
  & fr_silty_clay_loam, &
  & fr_clay_loam, &
  ...
  PUBLIC :: agg_hiressoil_data_to_target_grid

    CONTAINS

       !> Subroutine to aggregate hiressoil data to target grid
       SUBROUTINE agg_hiressoil_data_to_target_grid(tg,          &
                  &                   hiressoil_soil_unit,    &
                  &                   hiressoil_grid,         &
                  &                   lon_hiressoil,          &
                  &                   lat_hiressoil)
       ...
       INTEGER (KIND=i4), INTENT(IN) :: hiressoil_soil_unit(:,:) 
       TYPE(reg_lonlat_grid), INTENT(IN) :: hiressoil_grid ! grid of hiressoil data
       
       REAL (KIND=wp), INTENT(IN)  :: lon_hiressoil(:), &          
       & lat_hiressoil(:)          
      ...
      lat_loop: DO jr=1,hiressoil_grid%nlat_reg
      WRITE(message_text,*) 'jr = ',jr
      CALL logging%info(message_text)  
      raw_loop: DO ir=1,hiressoil_grid%nlon_reg
      ! find target data grid element index which is nearest to the raw data grid
      lon_pixel = lon_hiressoil(ir)
      lat_pixel = lat_hiressoil(jr)
      ...
      soil_unit = hiressoil_soil_unit(ir,jr)
      ...
      ENDIF
      IF (soil_unit == undef_hiressoiltype) then    ! ELSE
     ...
     ENDDO
   ENDDO
   END SUBROUTINE agg_hiressoil_data_to_target_grid
END MODULE mo_agg_hiressoil
