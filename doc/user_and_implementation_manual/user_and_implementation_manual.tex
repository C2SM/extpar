...
\author{
  ...
  Katie Osterried / Jonas Jucker / Philipp Sommer / J\"urgen Helmert
}  
...
\tabularnewline \hline
ground fraction covered by evergreen forest  &FOR\_\-E  &1  &GLC2000/Globcover/ ESA CCI-LC 
\tabularnewline \hline
...
The executable \textit{extpar\_landuse\_to\_buffer} aggregates the land use data to the target grid. Four different raw datasets can be processed: Globcover, GLC2000, GLCC and ESA CCI-LC; as GLC2000 and Globcover do not include Antarctica, GLCC or ESA CCI-LC data can be used for the missing areas.
\paragraph{Target grid definition}\ \par\medskip\noindent
...
In a next step the namelist 'INPUT\_LU' is read. It contains an integer switch (\textit{i\_landuse\_data}) that gives the possibility to choose between the four different raw data sets e.g., 1 (Globcover), 2 (GLC2000), 3 (GLCC) and 5 (ESA CCI-LC). For Globcover one can additionally choose to use the corine landuse dataset by setting the logical switch (\textit{l\_use\_corine}) to TRUE.  Furthermore the path and the filename of the desired raw data and of GLCC are specified there. The user must adjust the filename and path manually according to the chosen raw data in \textit{i\_landuse\_data}. In addition the name of the desired lookup table is read, which again can be chosen by the user using an integer switch \textit{ilookup\_table\_lu}. The lookup tables are described in more detail in table \ref{tab:look_up_table}. Finally, the names of the buffer files for the target landuse fields and for the target GLCC fields are specified. \par\medskip\noindent
...
\textbf{Modules name} & \textbf{Description}  \tabularnewline
\hline
\endhead
\hline
  mo\_logging &  Provides logging functionality\tabularnewline
  info\_extpar &  Contains information related to the EXTPAR (External Parameter) module\tabularnewline
  mo\_io\_units &  Manages input/output unit numbers\tabularnewline
  mo\_kind &  Defines constants for different kinds of data types\tabularnewline
  mo\_target\_grid\_data &  Contains data structures and functions related to the target grid\tabularnewline
  mo\_grid\_structures &  Contains data structures and functions related to different grid types (rotated\_lonlat\_grid, icosahedral\_triangular\_grid)\tabularnewline
  mo\_cosmo\_grid &  Provides functions for working with COSMO (COnsortium for Small scale MOdeling) grid\tabularnewline
  mo\_icon\_grid\_data &  Contains the definition of the ICON grid\tabularnewline
  mo\_base\_geometry &  Defines data types and functions for working with geographical and cartesian coordinates\tabularnewline
  mo\_additional\_geometry &  Contains additional geometric functions\tabularnewline
  mo\_icon\_domain &  Provides functions for working with ICON domain\tabularnewline
  mo\_math\_constants &  Defines mathematical constants\tabularnewline
  mo\_agg\_hwsdART &  Performs aggregation of hwsdART data to the target grid\tabularnewline
  mo\_read\_extpar\_namelists &  Reads namelists related to EXTPAR grid definition\tabularnewline
  mo\_hwsdART\_routines &  Contains functions for reading hwsdART data and namelists\tabularnewline
  mo\_hwsdART\_data &  Defines data types and constants related to hwsdART data\tabularnewline
  mo\_hwsdART\_tg\_fields &  Contains functions for allocating target fields for hwsdART\tabularnewline
  mo\_hwsdART\_output\_nc &  Provides functions for writing hwsdART data to NetCDF files\tabularnewline
  mo\_target\_grid\_routines &  Contains functions for initializing the target grid\tabularnewline
\bottomrule
\caption{Usage of modules in extpar\_hwsdART\_to\_buffer}
\label{tab:modules_hwsdART}
\end{longtable}
...
\hline
hwsdART\_buffer\_file & character & &  & name for hwsdART buffer file \tabularnewline
\hline
