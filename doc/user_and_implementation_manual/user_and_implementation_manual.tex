...
\author{
  ...
  Katie Osterried / Jonas Jucker / Jacopo Canton / Philipp Sommer
}  
...
\tabularnewline \hline
ground fraction covered by artificial (urban) areas  &URBAN  &1  &GLC2000/Globcover/ Ecoclimap/ESA CCI-LC/ LCZs with TERRA\_URB
\tabularnewline \hline
...
Urban shortwave albedo (mean facet-level) & URB\_SALB\_FL & 1     & LCZs with TERRA\_URB \tabularnewline\hline
Urban thermal albedo   (mean facet-level) & URB\_TALB\_FL & 1     & LCZs with TERRA\_URB \tabularnewline\hline
Urban emissivity       (mean facet-level) & URB\_EMIS\_FL & 1     & LCZs with TERRA\_URB \tabularnewline\hline
Urban shortwave albedo (bulk)             & URB\_SALB\_BK & 1     & LCZs with TERRA\_URB \tabularnewline\hline
Urban thermal albedo   (bulk)             & URB\_TALB\_BK & 1     & LCZs with TERRA\_URB \tabularnewline\hline
Urban emissivity       (bulk)             & URB\_EMIS\_BK & 1     & LCZs with TERRA\_URB \tabularnewline\hline
...
\textbf{Ecoclimap 2 (218)} &  & types not shown here \tabularnewline\hline
...
GLCC & 1 & operational settings of GME & Ritter, 2007 \tabularnewline
     & 2 & operational settings of COSMO & Heise, 2005 \tabularnewline
     & 3 & experimental settings, analog to lookup tables of ECOCLIMAP & Asensio, 2010 \tabularnewline\hline
Ecoclimap & n.a. & Ecoclimap lookup table & ecoclimap\_lookup.TAB \tabularnewline\hline
ESA CCI-LC & 1 & experimental settings & Helmert, 2019 \tabularnewline\hline
Ecoclimap-SG & 1 & Globcover analogue treating all LCZs as one unique class  & - \tabularnewline
             & 2 & Experimental settings with individual values for each LCZ & - \tabularnewline
\bottomrule
...
The executable \textit{extpar\_landuse\_to\_buffer} also includes the TERRA-URB module, controlled by the logical switch \textit{l\_terra\_urb}. This module uses a 2D map of local climate zones (LCZ) and aggregates the 2D variables used by TERRA-URB in COSMO/ICON (see table \ref{tab:terra_urb}) and outputs them to the lu\_buffer\_file. The aggregation procedure follows that of the other land use variables described in the previous sections. The TERRA-URB related variables then pass through the subprogram \textit{extpar\_consistency\_check} (see section \ref{extpar_consistency_check}) and are written out to the final extpar file.  If the ISA and or AHF extpar modules are also executed, their output is overwritten in the consistency check as the available ISA and AHF databases are of poorer quality compared to Ecoclimap-SG.
\begin{longtable}{ll}
...
\hline
i\_landuse\_data & integer & & &  switch to choose a land use raw data set; 1 Globcover2009, 2 GLC2000, 3 GLCC, 4 ECOCLIMAP, 5 ESA CCI-LC, 6 Ecoclimap-SG \tabularnewline
\hline
...
\tabularnewline
 ...
 & & & & 1: Globcover analogue treating all LCZs as one unique class\tabularnewline
 & & & & 2: Experimental settings with individual values for each LCZ\tabularnewline
\hline
