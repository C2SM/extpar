...
MODIS cloud droplet number climatology Q06 & 1 degree \tabularnewline\hline
\bottomrule
...
Monthly cloud droplet number from MODIS climatology & modis\_cdnc & $cm-3$ &  \tabularnewline\hline
\tabularnewline\hline
\bottomrule
...
\item \textbf{MODIS cdnc} \textit{first order conservative remapping}
...
\begin{itemize}
 ...
 \item data input: EDGARv6.1\_BC\_2018\_TOTALS.0.1x0.1.nc, EDGARv6.1\_OC\_2018\_TOTALS.0.1x0.1.nc, EDGARv6.1\_SO2\_2018\_TOTALS.0.1x0.1.nc
 \item Output: buffer file with EDGAR data (input\_edgar: edgar\_buffer\_file)
\end{itemize}

\subsection{extpar\_modis\_cdnc\_to\_buffer}\label{extpar_modis_cdnc_to_buffer}
\subsubsection{Short description of the subprogram \textit{extpar\_modis\_cdnc\_to\_buffer}}
The executable \textit{extpar\_modis\_cdnc\_to\_buffer} allows the interpolation of MODIS climatology data for cloud droplet number needed for the Cloud-Aerosol in ICON to the target grid. \par\medskip\noindent
The namelist contains only the path to the raw data, the raw data file names and the name of the buffer file.\par\medskip\noindent
The remapping to the target grid uses the \textit{first order conservative} interpolation. No other processing steps take place.

\subsubsection{Used namelist files and data in-/output:}
\begin{itemize}
 \item namelists files: namelist.py (dict: input\_modis\_cdnc), INPUT\_grid\_org, INPUT\_ICON\_GRID
 \item generate namelist: INPUT\_modis\_cdnc
 \item data input: modis\_cdnc\_climatology\_Q06.nc
 \item Output: buffer file with MODIS cloud droplet number data (input\_modis\_cdnc: modis\_cdnc\_buffer\_file)
\end{itemize}
...
\hline
INPUT\_modis\_cdnc & settings for MODIS cdnc data & extpar\_modis\_cdnc\_-to\_buffer & extpar\_consistency\_check \tabularnewline
\hline
...
\hline
raw\_data\_edgar\_\-filename\_so2 & character & &  & filename of EDGAR sulfur dioxide raw data
\tabularnewline
\hline 
\bottomrule
\end{longtable}

\subsection{MODIS cdnc data}\label{namelist_input_for_modis_cdnc}

\subsubsection*{DICT input\_modis\_cdnc (namelist.py)}
...
\endhead
\hline
raw\_data\_modis\_cdnc\_path & character & &  & path to MODIS cdnc raw data \tabularnewline
\hline
raw\_data\_modis_\cdnc\_\-filename & character & &  & filename of MODIS cdnc raw data \tabularnewline
\hline
\bottomrule
\end{longtable}
