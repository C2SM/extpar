\documentclass[a4paper,10pt,DIV14,BCOR1cm,titlepage]{scrartcl}  
...
\author{
        Hermann Asensio / Martina Messmer / Daniel L\"uthi / Katie Osterried
        }  
...
Numerical Weather Prediction (NWP) models and Climate models require geographical localized datasets like the topographic height of the earth surface, the plant cover, the distribution of land and sea and, dependent on the schemes used, a variety of other so called external parameters.\par\medskip\noindent
...
\item The target grid has to be specified, supported target grids are \begin{itemize}
...
These raw datasets can be found on the Linux-workstations and the hpc at DWD in the directory  \textasciitilde routfor/routfox/extpar/rawdata
\clearpage
...
\begin{longtable}{|p{6.5cm}|p{3cm}|p{1.0cm}|p{3.5cm}|}
\hline
\textbf{external parameter} &\textbf{short name} &\textbf{unit} &\textbf{raw dataset}  \tabularnewline
\hline
\endhead
\hline
geometrical height  &HSURF &$m$ &GLOBE/ASTER  \tabularnewline \hline
geopotential of earth surface &FIS &$ m^{2} s^{-1}$ &GLOBE/ASTER  \tabularnewline \hline
...
\tabularnewline \hline
fraction land cover  &FR\_\-LAND  & 1 &GLC2000/Globcover/ Ecoclimap 
\tabularnewline \hline
ground fraction covered by plants max (vegetation period) & PLCOV\_\-MX & 1 & GLC2000/Globcover/ Ecoclimap
\tabularnewline \hline
ground fraction covered by plants min (vegetation period) & PLCOV\_\-MN & 1 & GLC2000/Globcover/ EcoClimap
\tabularnewline \hline
ground fraction covered by artificial (urban) areas  &URBAN  &1  &GLC2000/Globcover/ Ecoclimap  
\tabularnewline \hline
ground fraction covered by evergreen forest  &FOR\_\-E  &1  &GLC2000/Globcover/ Ecoclimap 
\tabularnewline \hline
ground fraction covered by deciduous forest  &FOR\_\-D  &1  &GLC2000/Globcover/ Ecoclimap  
\tabularnewline \hline
root depth  &ROOTDP  &$m$  &GLC2000/Globcover/ Ecoclimap  
\tabularnewline \hline
leaf area index max(vegetation period)  &LAI\_\-MX  &1  &GLC2000/Globcover/ Ecoclimap  
\tabularnewline \hline
leaf area index min (vegetation period)  &LAI\_\-MN  &1  &GLC2000/Globcover/ Ecoclimap  
\tabularnewline \hline
plant resistance  &PRS\_\-MIN  &$s m^{-1}$  &GLC2000/Globcover/ Ecoclimap  
\tabularnewline \hline
long wave surface emissivity  &EMISS\_\-RAD  &1  &GLC2000/Globcover/ Ecoclimap  
\tabularnewline \hline
...
The software EXTPAR is composed of twelve autonomous programs. Eleven programs are responsible for aggregating a raw data to the target grid, which is specified by the user. The twelfth program, the consistency check, is performed in the end. The executables are called extpar\_$\ast$\_to\_buffer, whereas the star stands for ahf (anthropogenic heat flux), alb (albedo), aot (aerosol optical thickness), cru (temperature climatology of the Climate Research Unit (CRU)), landuse, topo (topography), ndvi (normalized difference vegetation index), soil, flake (fraction lake), sgsl (sub-grid scale slope), and isa (impervious surface area), respectively. In Fig. \ref{fig:EXTPAR_Figure} a schematic representation of EXTPAR is drawn. For the sake of clarity only the topography and land-use path is shown. The same can be applied for the other nine raw data sets. For all these programs there exist namelists. Most of the namelists only contain the name and path of the raw data file and the name of the buffer file, which is later used for the consistency check, and the name of the output of the final external variables. This is true for the following namelists:
'INPUT\_AOT', 'INPUT\_TCLIM' (it does temporarily contain a switch to choose between the old and new data set), 'INPUT\_NDVI', 'INPUT\_FLAKE' and 'INPUT\_ALB'. The latter does additionally include the variable names used in the raw data files.\par\medskip\noindent
The namelist 'INPUT\_LU' contains two integer switches which allow to choose between two raw data sets (GLC\_2000 and Globcover) and their lookup tables. But it also has an additional raw data file name and path for the single global raw land-use data set GLCC. It is the only one that contains the Antarctica and provides the information for the area below around 60 degrees south as the aforementioned land-use data sets are not able to provide any information there.\par\medskip\noindent
The namelist 'INPUT\_ORO' gives the possibility to switch between two raw orographical data sets (GLOBE and ASTER). It must be considered, that ASTER is not yet completely downloaded and is therefore not globally available. The downloaded region extends from 60\textdegree N to 60\textdegree S and from 180\textdegree W to 180\textdegree E. It is not recommended to derive the topographical parameters from ASTER if the region is beyond 60 degrees north or south. This is due to quality issues. The ASTER files are arranged as displayed in Fig. \ref{fig:ASTER_files}. As the computational time of the program \textit{extpar\_topo\_to\_buffer} depends mainly on the number of ASTER files that are read in, two new parameters are introduced in the namelist. These two parameters give the number of columns and rows of the used ASTER files. The filenames of the desired ASTER files must be given manually. Figure \ref{fig:ASTER_files} gives an example on how to use these parameters in the case of COSMO-2. If GLOBE is used the columns and rows are set to 4 and all GLOBE files must be listed in the \textit{topo\_files} parameter. A check in the program \textit{extpar\_topo\_to\_buffer} is introduced, which gives a warning if the borders of the domain are exceeded. This is followed by an abortion of this program. As there is no need to calculate the subgrid scale parameters (SSO) for high resolution setups, there is the logical switch \textit{lsso\_parm} to turn of the calculation of the SSOs.\par\medskip
...
\begin{pgfpicture}{0cm}{0cm}{16cm}{12cm}
 ...
 \end{pgfpicture}
\caption{\label{fig:ASTER_files}Illustration of the single domains of the 240 ASTER tiles. An example on how the three parameters ntiles\_columns, ntiles\_row and topo\_files in the namelist could look like is given in red.}	
\end{figure}
\noindent The namelist 'INPUT\_SCALE\_SEP' provides information needed to calculate the SSO parameters and roughness length based on a 3 km filtered topography. The logical switch lscale\_separation allows the user to decide whether a scale separation is desired or not. Note that the scale separation can only be applied for GLOBE and not for ASTER. Furthermore the user must decide whether the calculation of the SSO parameters make sense or not (see for this table \ref{tab:scale_separation}). \par\medskip\noindent
In the namelist 'INPUT\_OROSMOOTH' all parameters that are used to smooth the topography, as also implemented in Int2lm, are given. These values can be changed by the user, compare the namelist orography\_smoothing in chapter \ref{namelist_input_for_extpar_orography}. There is also a logical switch that allows the disabling of the orographical smoothing.\par\medskip\noindent
The third topographical based namelist is 'INPUT\_RADTOPO'. It contains a logical switch that controls the topographical corrected radiation parameters. Additionally the number of horizons must be specified. It is not recommended to deviate from 24. \par\medskip\noindent
The namelist 'INPUT\_SOIL' includes an integer switch, which allows to choose between the two data sets FAO (1) and HWSD (2). Because the HWSD data also provides deep soil types, there is a logical switch which can turn off these calculations. Be aware that the deep soil can only be calculated if the HWSD data are used for the topsoil. Additionally to the buffer and output file names, the path to the raw data files and the file names are specified. The latter must be manually changed for the topsoil if the raw data is changed from FAO to HWSD and vice versa. Next to these parameters the file names of the lookup tables for the HWSD data must be specified.\par\medskip\noindent
The namelists 'INPUT\_grid\_org' and either 'INPUT\_COSMO\_GRID' or 'INPUT\_ICON\_GRID' are used in all eleven programs, as they contain the general information of the target grid to which the raw data should be aggregated.\par\medskip\noindent
...
\end{figure}
\newpage
\paragraph{Summary} \ \par\medskip\noindent
The external parameters can be generated by using eleven programs to aggregate the various raw datasets to the target grid and after this by calling the final program for the important consistency check.
...
\item Then the aggregation of the raw datasets listed in table \ref{input_raw_data} to the given target grid can be performed by calling following executables
  \begin{itemize}
     ...
     \item extpar\_albedo\_to\_buffer
     \item extpar\_isa\_to\_buffer
     \item extpar\_ahf\_to\_buffer
     \item extpar\_sgsl\_to\_buffer
  \end{itemize}
...
\paragraph{Raw topography data}\ \par\medskip\noindent
...
\begin{itemize}
 \item namelists files: INPUT\_grid\_org, INPUT\_COSMO\_GRID, INPUT\_ICON\_GRID, INPUT\_ORO, INPUT\_OROSMOOTH
...
In a next step the namelist 'INPUT\_LU' is read. It contains an integer switch (\textit{i\_landuse\_data}) that gives the possibility to choose between the four different raw data sets e.g., 1 (Globcover), 2 (GLC2000), 3 (GLCC) and 4 (Ecoclimap). Furthermore the path and the filename of the desired raw data and of GLCC are specified there. The user must adjust the filename and path manually according to the chosen raw data in \textit{i\_landuse\_data}. In addition the name of the desired lookup table is read, which again can be chosen by the user using an integer switch \textit{ilookup\_table\_lu}. This switch has no effect if the Ecoclimap dataset is chosen. The lookup tables are described in more detail in table \ref{tab:look_up_table}. Finally, the names of the buffer files for the target landuse fields and for the target GLCC fields are specified. \par\medskip\noindent
...
\begin{itemize}
 ...
 \item data input:  glc2000\_byte.nc, glcc\_usgs\_class\_byte.nc, GLOBCOVER\_0\_16bit.nc, GLOBCOVER\_1\_16bit.nc, GLOBCOVER\_2\_16bit.nc, GLOBCOVER\_3\_16bit.nc, GLOBCOVER\_4\_16bit.nc, GLOBCOVER\_5\_16bit.nc, ecoclimap\_v2\_byte.nc             
...
The executable \textit{extpar\_cru\_to\_buffer} aggregates the temperature climatology of the Climate Research Unit (CRU) to the target grid.\par\medskip\noindent
\paragraph{Target grid definition} \ \par\medskip\noindent
The definition of the target grid is again done using the namelist 'INPUT\_grid\_org'. As the subroutines are exactly the same as the ones used in \textit{extpar\_topo\_to\_buffer}, it is referred to the subsection \textit{'Target grid definition'} in section \ref{extpar_topo_to_buffer}, where the procedure is explained in more detail. \par\medskip\noindent
\paragraph{Raw temperature climatology data}\ \par\medskip\noindent
The namelist 'INPUT\_TCLIM' gives the information of the path and the name of the raw temperature climatology data file. Additionally, the filenames for the buffer and output files are provided. There is also an integer switch (raw\_data\_t\_id), which allows to choose between a newer higher resolved data set (1) and the older coarser raw data set (2). The dimensions, such as the number of grid points in latitudinal and longitudinal direction and the number of months, are read from the NetCDF file directly. These dimensions can immediately be used to allocate the raw temperature climatology data. The grid is specified and defined from the NetCDF file. The last step before the aggregation can take place is to allocate the target fields. \par\medskip\noindent
\paragraph{Aggregation of the temperature climatology to the target field}\ \par\medskip\noindent
\textit{The following paragraphs describe computations on the raw data grid}\par\medskip\noindent
The temperature climatology is treated pointwise and therefore a loop over the latitude and the longitude is started. The raw data set contains a two dimensional field for every months, but a yearly temperature climatology is required. Thus the CRU raw data is summed up and weighted with the days per month and then normalized by the days in a year. Also the temperature is transformed from Celsius to Kelvin as necessary. (This must be done only for the older version of the CRU temperature, as the new one is already provided in Kelvin.)\par\medskip\noindent
\textit{The following paragraphs describe computations on the target grid}\par\medskip\noindent
As the resolution of the raw data set is so coarse, there is no need to go through the whole raw data set and find the corresponding target grid element. Here there is only one loop over the target grid. For every target grid element four surrounding raw data points are searched for. With these four points, a weight for the bilinear interpolation is computed. Four new arrays (SW, SE, NE, NW) are then defined, which contain the four neighbor values. These can now be used, together with the previously calculated weights, to calculate the bilinear interpolation. For the most recent CRU data set, an elevation parameter is also given. This parameter is aggregated in the same way as the temperature. The elevation is later needed in the consistency check to adjust the temperature to the height of the corresponding target grid element. The target values can now be written to a buffer and output file, and the allocated memory can be deallocated.
\subsubsection{Used namelist files and data in-/output:}
\begin{itemize}
\item namelists files: INPUT\_grid\_org, INPUT\_COSMO\_GRID, INPUT\_ICON\_GRID, INPUT\_TCLIM
 \item data input: absolute\_hadcrut3.nc, CRU\_T2M\_SURF\_clim.nc
 \item Output: crutemp\_clim\_extpar\_buffer.nc
\end{itemize}
...
The executable \textit{extpar\_ndvi\_to\_buffer} aggregates NDVI data (Normalized Differential Vegetation Index) to the target grid.\par\medskip\noindent
\paragraph{Target grid definition} \ \par\medskip\noindent
The definition of the target grid is again done using the namelist 'INPUT\_grid\_org'. As the subroutines are exactly the same as the ones used in \textit{extpar\_topo\_to\_buffer}, it is referred to the subsection \textit{'Target grid definition'} in section \ref{extpar_topo_to_buffer}, where the procedure is explained in more detail. \par\medskip\noindent
\paragraph{Raw NDVI data}\ \par\medskip\noindent
For the aggregation of the normalized differential vegetation index the namelist is quite simple again. It contains the path and the filename of the raw data set, as well as the names of the buffer and output files for the consistency check.\par\medskip\noindent
In order to be able to allocate the raw data, its dimensions must first be known. Therefore the number of grid points in x- and y-direction and the number of time in months are read from the netcdf file directly. Having these varibles the raw data fields can be allocated. Additionally, the target fields must be allocated, which is done using the target grid information and the number of months. The number of months is twelve, as a yearly cycle is calculated. Before the aggregation can start the grid of the raw data must be defined. Especially the start latitude and longitude as well as the resolution in both directions is needed. \par\medskip\noindent
\paragraph{Aggregation of the raw data to the target grid}\ \par\medskip\noindent
\textit{The following paragraphs describe computations on the raw data grid}\par\medskip\noindent
To aggregate the data three loops are necessary: The first one over the time index, the second over the latitudes, where the data is read from the NetCDF file row-wise from 180 degrees west to 180 degrees east and finally over the longitude, which indicates a point-wise treatment of the raw data. For each raw data grid element a corresponding target grid element is defined. All the raw data pixels that are closer than half a grid point away from the target grid are aggregated to exactly this target grid element. This routine is described in more detail in the subsection \textit{'Aggregation of the raw topography to the target grid'} in section \ref{extpar_topo_to_buffer} and in Fig. \ref{fig:aggregation_figure}. Each time a raw data point is associated with a target grid, the number of raw data points is increased by one and the ndvi field is summed up. \par\medskip\noindent 
\textit{The following paragraphs describe computations on the target grid}\par\medskip\noindent
If there is at least one raw data element in a target grid element, the summed up ndvi field is normalized by the number of available raw data points. If no raw data point is available, a weighted bilinear interpolation is performed using four neighbor raw data pixels. In addition to the ndvi value, the monthly ratio with respect to the maximum ndvi is computed. \par\medskip\noindent
Finally the data is saved in a NetCDF buffer and an output file and the allocated data is released again.
\subsubsection{Used namelist files and data in-/output:}
\begin{itemize}
 \item namelists files: INPUT\_grid\_org, INPUT\_COSMO\_GRID, INPUT\_ICON\_GRID, INPUT\_NDVI
 \item data input: NDVI\_1998\_2003.nc
 \item Output: NDVI\_buffer.nc
\end{itemize}
...
\subsection{extpar\_albedo\_to\_buffer}\label{extpar_albedo_to_buffer}
\subsubsection{Short description of the subprogram \textit{extpar\_albedo\_to\_buffer}}
The executable \textit{extpar\_albedo\_to\_buffer} allows the aggregation of two different kinds of albedo data to the target grid. The first kind is a climatology of monthly values of total albedo derived from MODIS satellite data for the 3 spectral bands visible, near infrared and ultraviolet. The second kind contains information for soil albedo only in dry and saturated conditions. It originates from the Community Land Model\footnote{https://svn-ccsm-inputdata.cgd.ucar.edu/trunk/inputdata/lnd/clm2/rawdata/mksrf\_soilcol.081008.nc \\ Lawrence, P. J. and T. N. Chase (2007). "Representing a new MODIS consistent land surface in the Community Land Model (CLM 3.0)." Journal of Geophysical Research-Biogeosciences 112(G1). \\Table 3.3 in: Oleson, K.W., D.M. Lawrence, G.B. Bonan, M.G. Flanner, E. Kluzek, P.J. Lawrence, S. Levis, S.C. Swenson, P.E. Thornton, A. Dai, M. Decker, R. Dickinson, J. Feddema, C.L. Heald, F. Hoffman, J.-F. Lamarque, N. Mahowald, G.-Y. Niu, T. Qian, J. Randerson, S. Running, K. Sakaguchi, A. Slater, R. Stockli, A. Wang, Z.-L. Yang, Xi. Zeng, and Xu. Zeng, 2010: Technical Description of version 4.0 of the Community Land Model (CLM). NCAR Technical Note NCAR/TN-478+STR, National Center for Atmospheric Research, Boulder, CO, 257 pp.}.\par\medskip\noindent
\paragraph{Target grid definition} \ \par\medskip\noindent
The definition of the target grid is again done using the namelist 'INPUT\_grid\_org'. As the subroutines are exactly the same as the ones used in \textit{extpar\_topo\_to\_buffer}, it is referred to the subsection \textit{'Target grid definition'} in section \ref{extpar_topo_to_buffer}, where the procedure is explained in more detail. \par\medskip\noindent
\paragraph{Raw albedo data}\ \par\medskip\noindent
The namelist 'INPUT\_ALB' defines the filenames and paths for the normal albedo, the near infrared albedo and the ultra violet albedo. Also the names for the buffer and the output file are given. Additionally, the variable names to extract from each NetCDF file are specified. The integer switch \textit{ialb\_type} determines which albedo fields are produced: 1 (MODIS monthly albedo), 2 (only dry and saturated soil albedo) or 3 (only visible band of monthly MODIS data).\par\medskip\noindent
As the three data sets do all have the same grid, the dimension is only inquired once. This information allows the allocation of all three raw albedo fields. The required information is read from one netcdf file (the start latitude and longitude and the resolution in both directions). All the target fields are also allocated at once, using the target grid and the number of months that need to be calculated, which is 12. \par\medskip\noindent
\paragraph{Aggregation of the albedo raw data to the target grid} \ \par\medskip\noindent
The aggregation of the data is done in the exact same way for the three data sets: albedo, near infrared albedo and ultra violet albedo. The data is read from the netcdf file row-wise and than treated point-wise, as each raw data grid point is assigned to a target grid point. The association of a raw data point and a target grid element is done only for the first time step, as it stays the same for all the others. The albedo values of each raw data point that contributes to a target grid element are summed up, and the number of these raw data pixels is added up. At target points that are associated with at least one raw data point, the albedo is obtained by normalizing the previously computed sum with the number of associated raw data pixels. If there is no raw data pixel available a weighted bilinear interpolation is performed.\par\medskip\noindent
As soon as the aggregation is done for all three data sets the target fields are written to the buffer and output netcdf file. The allocated memory is then deallocated.\par\medskip\noindent

\subsubsection{Used namelist files and data in-/output:}
\begin{itemize}
 \item namelists files: INPUT\_grid\_org, INPUT\_COSMO\_GRID, INPUT\_ICON\_GRID, INPUT\_ALB
 \item data input: month\_alb.nc, month\_alnid.nc, month\_aluvd.nc, global\_soil\_albedo.nc
 \item Output: month\_alb\_buffer.nc
\end{itemize}
...
The executable \textit{extpar\_ahf\_to\_buffer} allows to aggregate or interpolate data on the anthropogenic heat flux needed by TERRA\_URB to the target grid. \par\medskip\noindent
\paragraph{Target grid definition} \ \par\medskip\noindent
The definition of the target grid is again done using the namelist 'INPUT\_grid\_org'. As the subroutines are exactly the same as the ones used in \textit{extpar\_topo\_to\_buffer}, it is referred to the subsection \textit{'Target grid definition'} in section \ref{extpar_topo_to_buffer}, where the procedure is explained in more detail. \par\medskip\noindent
...
\subsection{extpar\_sgsl\_to\_buffer}\label{extpar_sgsl_to_buffer}
\subsubsection{Short description of the subprogram \textit{extpar\_sgsl\_to\_buffer}}
The program \textit{extpar\_sgsl\_to\_buffer} aggregates the subgrid-scale slope values derived from either the GLOBE or the ASTER dataset to the target grid.
\paragraph{Raw subgrid scale slope parameter data}\ \par\medskip\noindent
The variables of the namelist 'INPUT\_SGSL', which cover all the raw subgrid-scale slope parameter data information, is fed into the program. In this namelist the path of the raw data is given as well as the names of the subgrid-scale slope data files. An integer switch allows the choice between the highly resolved, non-global DEM ASTER based and the coarser but global DEM GLOBE based raw data (1: GLOBE, 2: ASTER). In order to define the right number of raw data tiles the variables ntiles\_column and ntiles\_row must be available in the namelist. Additionally, the name for the buffer is defined.\par\medskip\noindent
The subgrid-scale slope data files must be manually changed in the run script, when switching from GLOBE to ASTER and vice versa.\par\medskip\noindent
Further the number of tiles of the raw topography data is defined (this depends on the DEM used:  1-16 tiles for GLOBE and 1 - 240 tiles for ASTER). This value is the product of ntiles\_column and ntiles\_row. The further processing follows exactly the one for the orographic height data.\par\medskip\noindent
\subsubsection{Used namelist files and data in-/output:}
\begin{itemize}
 \item namelists files: INPUT\_grid\_org, INPUT\_COSMO\_GRID, INPUT\_ICON\_GRID, INPUT\_SGSL
\item data input (GLOBE): S\_ORO\_A10.nc, S\_ORO\_B10.nc, S\_ORO\_C10.nc, S\_ORO\_D10.nc, S\_ORO\_E10.nc, S\_ORO\_F10.nc, S\_ORO\_G10.nc, S\_ORO\_H10.nc, S\_ORO\_I10.nc, S\_ORO\_J10.nc, S\_ORO\_K10.nc, S\_ORO\_L10.nc, S\_ORO\_M10.nc, S\_ORO\_N10.nc, S\_ORO\_O10.nc, S\_ORO\_P10.nc 
\item data input (ASTER): S\_ORO\_T001.nc - S\_ORO\_T240.nc
\item Output: GLOBE\_buffer.nc
\end{itemize}
...
An additional namelist that is used, is the 'INPUT\_CHECK', which defines all the buffer files. These files are read and all the variables needed for the final external parameters file are allocated.\par\medskip\noindent
...
The consistency check of the temperature climatology contains a height correction and is only performed for the finer resolved temperature climatology (e.g. raw\_data\_t\_id = 1). The temperature is set to undefined for all the sea points. For land points that have a valid temperature, it is adjusted to the height. This is done by considering a constant temperature rate of 0.65 K per 100m ($\frac{dT}{dh} = -\frac{0.65 K}{100 m}$). \par\medskip\noindent
...
\item The OpenMP implementation is not yet fully tested. 
...
\hline
  INPUT\_TCLIM  & settings for temperature data     & runscript & extpar\_cru\_to\_buffer  \\
\hline 
INPUT\_NDVI & settings for NDVI data & runscript & extpar\_ndvi\_to\_buffer \\
\hline 
...
\hline
INPUT\_ALB & settings for albedo data & runscript & extpar\_albedo\_to\_buffer \\
\hline
...
\hline
 INPUT\_SGSL & settings for subgrid-scale slope parameter & runscript & extpar\_sgsl\_to\_buffer \\
\hline
...
\subsubsection*{NAMELIST /gridref\_ini/ (INPUT\_ICON\_GRID)}
\begin{longtable}{|p{3cm}|p{1.6cm}|p{1.5cm}|p{0.75cm}|p{4.5cm}|p{2.25cm}|}
\hline 
\textbf{Parameter}&
\textbf{Type}&
\textbf{Default}&
...
\subsubsection*{NAMELIST /icon\_grid\_ctl/ (INPUT\_ICON\_GRID) }
The namelist /icon\_grid\_files/ specifies the filenames and the directory of the Icon grid files with the coordinates of the Icon grid.
...
\endhead
\hline 
icon\_dom\_nr & I & &  & number of icon domain for target grid \tabularnewline
\hline 
grid\_level & I & &  & level of current grid \tabularnewline
\hline 
grid\_id & I & &  &  domain id of current grid \tabularnewline
\hline 
i\_cell\_type & I & 3 &  &  Cell shape. 3: triangluar grid, 4: quadrilateral grid, 6: hexagonal/pentagonal grid \tabularnewline
\hline 
\bottomrule
\end{longtable}


\subsubsection*{NAMELIST /icon\_grid\_files/ (INPUT\_ICON\_GRID) }
The namelist /icon\_grid\_files/ specifies the filenames and the directory of the Icon grid files with the coordinates of the Icon grid.
...
\endhead
\hline 
icon\_grid\_dir & character & &  & path to directory which contains the ICON grid files with the coordinates \tabularnewline
\hline 
icon\_grid\_nc\_files & character (max\_dom) & & & filenames of the ICON grid files with the coordinates\tabularnewline
\hline 
\bottomrule
\end{longtable}
...
\endhead
\hline 
itopo\_type & integer& & & switch to choose an orography raw data set; 1 GLOBE, 2 ASTER
\tabularnewline
\hline
...
\tabularnewline
\hline
ntiles\_column & integer & GLOBE: 4 \newline ASTER: x & & number of tile columns of desired region
\tabularnewline
\hline
ntiles\_row & integer & GLOBE: 4 \newline ASTER: x & & number of tile rows of desired region
\tabularnewline
\hline
topo\_files & character & &  &  filenames of GLOBE (16 tiles) / ASTER (240 tiles) raw data sets
\tabularnewline
\hline
lsubtract\_mean\_slope & logical & &  &  
\tabularnewline
\hline
\bottomrule
\end{longtable}
...
\tabularnewline
\hline 
orography\_output\_file & character & &  & name for orography output file \tabularnewline
\hline
\bottomrule
\end{longtable}
...
\hline
i\_landuse\_data & integer & & &  switch to choose a land use raw data set; 1 Globcover2009, 2 GLC2000, 3 GLCC, 4 ECOCLIMAP \tabularnewline
\hline
...
\subsubsection*{NAMELIST /t\_clim\_raw\_data/ (INPUT\_TCLIM)}
...
\hline 
raw\_data\_t\_clim\_filename & character & &  & filename of T2m climatology  data \tabularnewline
\hline
raw\_data\_t\_id & integer & & & switch to choose between the new and fine (1) and the old and coarse (2) raw data set. Note that the fine data set (1) is topographically corrected.\tabularnewline
\hline
\bottomrule
\end{longtable}

\subsubsection*{NAMELIST /t\_clim\_io\_extpar/ (INPUT\_TCLIM)}
\begin{longtable}{|p{4cm}|p{1.5cm}|p{1.5cm}|p{1cm}|p{6cm}|}
\hline 
\textbf{Parameter}& \textbf{Type}& \textbf{Default}& \textbf{Unit}& \textbf{Description}
\tabularnewline
...
\tabularnewline
\hline 
t\_clim\_output\_file & character & &  & name for t\_clim output file \tabularnewline
\hline
\bottomrule
\end{longtable}
...
\subsubsection*{NAMELIST /ndvi\_raw\_data/ (INPUT\_NDVI)}
...
\subsubsection*{NAMELIST /ndvi\_io\_extpar/ (INPUT\_NDVI)}
\begin{longtable}{|p{4cm}|p{1.5cm}|p{1.5cm}|p{1cm}|p{6cm}|}
\hline 
\textbf{Parameter}& \textbf{Type}& \textbf{Default}& \textbf{Unit}& \textbf{Description}
\tabularnewline
...
\tabularnewline
\hline 
ndvi\_output\_file & character & &  & name for ndvi output file \tabularnewline
\hline
\bottomrule
\end{longtable}
...
\tabularnewline
\hline 
soil\_output\_file & character & &  & name for soil output file \tabularnewline
\hline
...
\tabularnewline
\hline 
flake\_output\_file & character & &  & name for flake output file \tabularnewline
\hline
\bottomrule
\end{longtable}
...
\subsubsection*{NAMELIST /alb\_raw\_data/ (INPUT\_ALB)}
...
\subsubsection*{NAMELIST /alnid\_raw\_data/ (INPUT\_ALB)}
...
\tabularnewline
\hline
raw\_data\_alnid\_filename & character & & & filename of the raw near infrared albedo data
\tabularnewline
\hline
\bottomrule
\end{longtable}

\subsubsection*{NAMELIST /aluvd\_raw\_data/ (INPUT\_ALB)}
...
\tabularnewline
\hline
raw\_data\_aluvd\_filename & character & & & filename of the raw ultra violet albedo data
\tabularnewline
\hline
\bottomrule
\end{longtable}

\subsubsection*{NAMELIST /alb\_io\_extpar/ (INPUT\_ALB)}
...
\tabularnewline
\hline
alb\_output\_file & character & & & name for the albedo output file
\tabularnewline
\hline
\bottomrule
\end{longtable}

\subsubsection*{NAMELIST /alb\_source\_file/ (INPUT\_ALB)}
...
\tabularnewline
\hline
alnid\_source & character & 'alnid' & & variable name of the near infrared albedo netcdf file
\tabularnewline
\hline
aluvd\_source & character & 'aluvd' & & variable name of the ultra violet albedo netcdf file
\tabularnewline
\hline
\bottomrule
\end{longtable}
...
\subsection{Subgrid-scale slope parameter}\label{namelist_input_for_extpar_sgsl}

\subsubsection*{NAMELIST /sgsl\_raw\_data/ (INPUT\_SGSL)}
...
\endhead
\hline 
idem\_type & integer& & & switch to DEM data set; 1 GLOBE, 2 ASTER
\tabularnewline
\hline
raw\_data\_sgsl\_path & character & &  & path to slope parameter raw data
\tabularnewline
\hline
ntiles\_column & integer & GLOBE: 1-4 \newline ASTER: 1-12 & & number of tile columns of desired region
\tabularnewline
\hline
ntiles\_row & integer & GLOBE: 1-4 \newline ASTER: 1-20 & & number of tile rows of desired region
\tabularnewline
\hline
...
\subsubsection*{NAMELIST /sgsl\_io\_extpar/ (INPUT\_SGSL)}
\begin{longtable}{|p{4cm}|p{1.5cm}|p{1.5cm}|p{1cm}|p{6cm}|}
\hline 
\textbf{Parameter}& \textbf{Type}& \textbf{Default}& \textbf{Unit}& \textbf{Description}
\tabularnewline
...
\endhead
\hline
sgsl\_buffer\_file & character & &  & name for subgrid-scale slope parameter buffer file
\tabularnewline
\hline 
\bottomrule
\end{longtable}
...
\endhead
\hline 
grib\_output\_filename & character & external\_parameters.grb &  & filename for grib output filename \tabularnewline
\hline 
grib\_sample & character & GRIB2.tmpl &  &  name for grib sample for grib\_api (sample to be found in \$GRIB\_SAMPLES\_PATH) \tabularnewline
\hline 
netcdf\_output\_filename & character & &  & filename for netcdf output filename \tabularnewline
\hline 
orography\_buffer\_file & character & &  & name for orography buffer file \tabularnewline
\hline 
soil\_buffer\_file & character & &  & name for soil buffer file \tabularnewline
\hline 
lu\_buffer\_file & character & &  & name for landuse buffer file \tabularnewline
\hline 
glcc\_buffer\_file & character & &  & name for glcc buffer file \tabularnewline
\hline 
flake\_buffer\_file & character & &  & name for flake buffer file \tabularnewline
\hline 
ndvi\_buffer\_file & character & &  & name for ndvi buffer file \tabularnewline
\hline 
sst\_icon\_file & character & &  & name for ICON sst buffer file \tabularnewline
\hline 
t\_2m\_icon\_file & character & &  & name for ICON two meter temperature buffer file \tabularnewline
\hline
t\_clim\_buffer\_file & character & &  & name for t\_clim buffer file \tabularnewline
\hline 
aot\_buffer\_file & character & &  & name for aot buffer file \tabularnewline
\hline
alb\_buffer\_file & character & &  & name for albedo buffer file \tabularnewline
\hline
...
\hline
lflake\_correction & logical & T & & flag indicating whether the additional correction for lake points bordering the ocean is desired. \tabularnewline
\hline
tile\_mode & integer & 0 & & 
\tabularnewline
\hline
\bottomrule
\end{longtable}
