...
class Test:
    ...
    def __prepare_print(self):
        """print infos about the upcoming test at the start"""
        self.logger.info('')
        self.logger.important('TEST {0}/{1}: {2}'.format(
    ...
    def prepare(self):
        ...
        if 'extpar' not in self.executable:
    ...
    def start(self):
        ...
        if self.options.mpicmd == '':
            ...
            if '&NTASKS' in self.options.mpicmd:
                ...
                run_cmd = self.options.mpicmd.replace('&NTASKS',
                                                      str(self.nprocs))
            else:
    ...
    def write_result(self):
        ...
        self.logger.result(
            0, self.result,
            'RESULT %s/%s: %s' % (self.type, self.name, self.description))
    ...
    def update_namelist(self):
        ...
        if re.match(pattern, text):
            status = change_dir(self.rundir, self.logger)
            self.logger.important('Updating namelist data/' + self.type + '/' +
                                  self.name)
    ...
    def __setup_directory(self):
        ...
        status = system_command('/bin/cp -f ' + self.namelistdir + '*.py .',
                                self.logger)

        # copy the icon grid if exists
        if not glob.glob(dir_path(self.namelistdir) + 'icon_grid_*') == []:
            status = system_command(
                '/bin/cp -f ' + self.namelistdir + 'icon_grid_* ./',
                self.logger)

        # copy of the climatological fields needed for the icon test if exists
        if not glob.glob(dir_path(self.namelistdir) + 'clim_*') == []:
            status = system_command(
                '/bin/cp -f ' + self.namelistdir + 'clim_* ./', self.logger)
    ...
    def __setup_executable(self):
        ...
        if self.executable == None:
            raise SkipError(
                'No executable defined in argument list or xml file')

        self.logger.info('Fetching executable ' + self.basedir +
                         self.executable)

        # copy of the executable
        if 'extpar' in self.executable:
            if not os.path.exists(self.basedir + 'bin/' + self.executable):
                raise SkipError('Executable ' + self.basedir + 'bin/' +
                                self.executable + ' does not exist')
        else:
            if not os.path.exists(self.basedir + self.executable):
                raise SkipError('Executable ' + self.basedir +
                                self.executable + ' does not exist')
        if 'extpar' in self.executable:
            status = system_command('/bin/cp ' + self.basedir + 'bin/* .',
                                    self.logger)
        else:
            status = system_command(
                '/bin/cp ' + self.basedir + self.executable + ' .',
                self.logger)
    ...
    def __set_pert(self):
        ...
        if self.conf.pert_avail == 'True':
            ...
            replace_param(self.conf.par_file, 'itype_pert',
                          ' itype_pert = %i' % pert)
    ...
    def set_parallelization(nprocs, nprocio):
        ...
        parlist = sorted(parlist,
                         key=lambda tuple: aspect_ratio(tuple[0], tuple[1]))
